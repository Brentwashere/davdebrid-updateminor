...
export default class DebridLink extends Debrid {
    ...
    async getFiles(maxPages, perPage){
        ...
        // Safety check if pagination block is missing on small accounts
        const totalPages = res.pagination && res.pagination.pages ? res.pagination.pages : 1;
        const results = [res];

        // 2. Fetch remaining pages sequentially
        const lastPageTarget = Math.min(totalPages, maxPages) - 1;
        ...
        return results.reduce((files, res) => {
            if (!res.value || !Array.isArray(res.value)) return files;
